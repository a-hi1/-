...
<c:set var="pageTitle" value="修改批次信息" />
...
<div class="card" style="padding:0;overflow:hidden;">
  <form method="post" action="${pageContext.request.contextPath}/admin/teams/update">
    <input type="hidden" name="id" value="${team.id}" />
    <table class="table" style="margin:0;">
      <tr>
        <th style="width:220px;">批次名称</th>
        <td><input name="name" value="${team.name}" required style="max-width:300px;" /></td>
      </tr>
      <tr>
        <th>批次创建时间</th>
        <td><input name="beginYear" type="date" value="${team.beginYear}" required style="max-width:220px;" /></td>
      </tr>
      <tr>
        <th>批次状态</th>
        <td>
          <label style="margin-right:14px;">
            <input type="radio" name="status" value="2" <c:if test="${team.status == 2}">checked</c:if> /> 禁用
          </label>
          <label>
            <input type="radio" name="status" value="1" <c:if test="${team.status == 1 || empty team.status}">checked</c:if> /> 正常
          </label>
        </td>
      </tr>
      <tr>
